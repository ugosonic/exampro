...
class AdminRepository {
  ...
  Future<int> createCategory(
    ...
    if (_adminApi != null) {
      final id = await _adminApi.createCategory(
        name: name,
        order: order,
        imageUrl: imageUrl,
      );
  ...
  Future<int> createSubcategory(
    ...
    if (_adminApi != null) {
      final id = await _adminApi.createSubcategory(
    ...
    await (_db.update(_db.exams)..where((e) => e.id.equals(examId))).write(
      ExamsCompanion(
        ...
        subcategoryId: subcategoryId != null
            ? drift.Value(subcategoryId)
            : const drift.Value.absent(),
        timeLimitMinutes: timeLimitMinutes != null
            ? drift.Value(timeLimitMinutes)
            : const drift.Value.absent(),
        passPercent: passPercent != null
            ? drift.Value(passPercent)
            : const drift.Value.absent(),
        shuffleOptions: shuffleOptions != null
            ? drift.Value(shuffleOptions)
            : const drift.Value.absent(),
        negativeMarking: negativeMarking != null
            ? drift.Value(negativeMarking)
            : const drift.Value.absent(),
        published: published != null
            ? drift.Value(published)
            : const drift.Value.absent(),
        themeKey: themeKey != null
            ? drift.Value(themeKey)
            : const drift.Value.absent(),
        pdfUrl: pdfUrl != null
            ? drift.Value(pdfUrl)
            : const drift.Value.absent(),
    ...
    if (_adminApi != null) {
      ...
      try {
        await (_db.update(_db.categories)..where((c) => c.id.equals(id))).write(
          CategoriesCompanion(locked: drift.Value(locked)),
        );
      } catch (_) {}
      return;
    }
  ...
  Future<int> addQuestionWithOptions({
    ...
    final isMultiple = multiple ?? options.where((o) => o.correct).length > 1;
    if (_adminApi != null) {
      return _adminApi.addQuestionWithOptions(
        examId: examId,
        text: text,
    ...
    return await _db.transaction(() async {
      final qId = await _db
          ...
          .insert(
            QuestionsCompanion.insert(
              ...
              multiple: drift.Value(isMultiple),
            ),
          );
      await _db.batch((b) {
      ...
      await (_db.update(_db.exams)..where((e) => e.id.equals(examId))).write(
        ExamsCompanion(questionCount: drift.Value(count)),
      );
      return qId;
    });
  }
  ...
  Stream<List<Subcategory>> watchSubcategories(int categoryId) {
    ...
    return (_db.select(
      _db.subcategories,
    )..where((s) => s.categoryId.equals(categoryId))).watch();
  }

  Future<List<Subcategory>> allSubcategories() async =>
      _db.select(_db.subcategories).get();
  ...
  Future<void> updateSubcategory(
    ...
    await (_db.update(_db.subcategories)..where((s) => s.id.equals(id))).write(
      SubcategoriesCompanion(
        name: name != null ? drift.Value(name) : const drift.Value.absent(),
        order: order != null ? drift.Value(order) : const drift.Value.absent(),
        imageUrl: imageUrl != null
  ...
  Future<void> deleteSubcategory(int id) async {
    if (_adminApi != null) {
      await _adminApi.deleteSubcategory(id);
      try {
        await (_db.delete(
          _db.subcategories,
  ...
  Future<List<Category>> categoriesForQuestion(int questionId) async {
    final joins = await (_db.select(
      _db.questionCategories,
    )..where((qc) => qc.questionId.equals(questionId))).get();
    if (joins.isEmpty) return [];
    final catIds = joins.map((e) => e.categoryId).toList();
    return await (_db.select(
      _db.categories,
    )..where((c) => c.id.isIn(catIds))).get();
  }

  Future<void> setCategoriesForQuestion(
    int questionId,
    ...
    await _db.transaction(() async {
      await (_db.delete(
        _db.questionCategories,
      )..where((qc) => qc.questionId.equals(questionId))).go();
  ...
  Future<List<Subcategory>> subcategoriesForQuestion(int questionId) async {
    ...
    )..where((qs) => qs.questionId.equals(questionId))).get();
    if (joins.isEmpty) return [];
  ...
  Future<void> setSubcategoriesForQuestion(
    int questionId,
    ...
    await _db.transaction(() async {
      await (_db.delete(
        _db.questionSubcategories,
      )..where((qs) => qs.questionId.equals(questionId))).go();
  ...
  Future<void> deleteQuestion(int questionId) async {
    await _db.transaction(() async {
      ...
      await (_db.delete(
        _db.questions,
      )..where((t) => t.id.equals(questionId))).go();
    });
  }
  ...
  Future<void> setExamPublished(int examId, bool published) async {
    await (_db.update(_db.exams)..where((t) => t.id.equals(examId))).write(
      ExamsCompanion(published: drift.Value(published)),
    );
  }
  ...
  Future<void> deleteExam(int examId) async {
    ...
    await _db.transaction(() async {
      ...
      final attemptsForExam = await (_db.select(
        _db.attempts,
      )..where((a) => a.examId.equals(examId))).get();
      final attemptIds = attemptsForExam.map((a) => a.id).toList();
      if (attemptIds.isNotEmpty) {
        await (_db.delete(
          _db.attemptAnswers,
        )..where((aa) => aa.attemptId.isIn(attemptIds))).go();
        await (_db.delete(
          _db.attempts,
        )..where((a) => a.id.isIn(attemptIds))).go();
      }
      await (_db.delete(
        _db.examQuestions,
      )..where((eq) => eq.examId.equals(examId))).go();
      await (_db.delete(
        _db.examGradeBands,
      )..where((g) => g.examId.equals(examId))).go();
      await (_db.delete(
        _db.reports,
  ...
  Future<List<({Question question, List<Choice> options, ExamQuestion join})>>
  examQuestions(int examId) async {
    final joins =
    ...
    final qs = await (_db.select(
      _db.questions,
    )..where((q) => q.id.isIn(qIds))).get();
    final opts = await (_db.select(
      _db.choices,
    )..where((o) => o.questionId.isIn(qIds))).get();
    return [
  ...
  Future<void> reorderExamQuestions(int examId, List<int> questionIds) async {
    await _db.transaction(() async {
      for (var i = 0; i < questionIds.length; i++) {
        await (_db.update(_db.examQuestions)..where(
              (t) =>
                  t.examId.equals(examId) & t.questionId.equals(questionIds[i]),
            ))
  ...
  Future<void> removeQuestionFromExam(int examId, int questionId) async {
    if (_adminApi != null) {
      await _adminApi.deleteQuestionFromExam(
        examId: examId,
        questionId: questionId,
      );
      return;
    }
    await (_db.delete(_db.examQuestions)..where(
  ...
  Future<void> updateQuestionAndOptions({
    ...
    final isMultiple = options.where((o) => o.correct).length > 1;
    if (_adminApi != null) {
      await _adminApi.updateQuestionAndOptions(
        questionId: questionId,
        body: body,
        explanation: explanation,
        multiple: isMultiple,
    ...
    await _db.transaction(() async {
      await (_db.update(
        _db.questions,
      )..where((q) => q.id.equals(questionId))).write(
        QuestionsCompanion(
      ...
      await (_db.delete(
        _db.choices,
      )..where((o) => o.questionId.equals(questionId))).go();
      await _db.batch((b) {
        b.insertAll(_db.choices, [
          for (var i = 0; i < options.length; i++)
            ChoicesCompanion.insert(
  ...
  Future<({Question question, List<Choice> options})?> questionWithOptions(
    int questionId,
  ) async {
    final q = await (_db.select(
      _db.questions,
    )..where((r) => r.id.equals(questionId))).getSingleOrNull();
    if (q == null) return null;
    final options =
        await (_db.select(_db.choices)
  ...
  Future<void> setQuestionLocked(int id, bool locked) async {
    await (_db.update(_db.questions)..where((q) => q.id.equals(id))).write(
      QuestionsCompanion(locked: drift.Value(locked)),
    );
  }
    ...
    return await _db
        ...
        .insert(
          PaymentsCompanion.insert(
            ...
            currency: currency,
            stripePaymentIntentId: drift.Value(intentId),
  ...
  Future<void> markRefunded(int id) async {
    await (_db.update(_db.payments)..where((p) => p.id.equals(id))).write(
      PaymentsCompanion(
        refunded: const drift.Value(true),
        status: const drift.Value('refunded'),
      ),
    );
  }
  ...
  Future<void> setExamReadOnly(int examId, bool readOnly) async {
    ...
    await _db
        ...
        .insertOnConflictUpdate(
          AppSettingsCompanion(
            key: drift.Value(key),
            value: drift.Value(readOnly ? '1' : '0'),
          ),
        );
  }

  // Reports
  Future<List<({Report report, Exam exam})>> listReports({
    ...
    final exams = await (_db.select(
      _db.exams,
    )..where((e) => e.id.isIn(examIds))).get();
    return [
      for (final r in reps)
        (report: r, exam: exams.firstWhere((e) => e.id == r.examId)),
    ];
  }
  ...
  Future<void> markReportResolved(int id, bool resolved) async {
    await (_db.update(_db.reports)..where((r) => r.id.equals(id))).write(
      ReportsCompanion(resolved: drift.Value(resolved)),
    );
  }

  // Paging questions
  Future<List<Question>> pagedQuestions({
    required int limit,
    required int offset,
  }) async {
    final q = _db.select(_db.questions)
  ...
  Future<List<Question>> unassignedQuestions({
    int limit = 500,
    int offset = 0,
  }) async {
    final res = await _db
  ...
  Future<List<Question>> questionsInCategory(
    int categoryId, {
    int limit = 500,
    int offset = 0,
  }) async {
  ...
  Future<List<Question>> questionsInSubcategory(
    int subcategoryId, {
    int limit = 500,
    int offset = 0,
  }) async {
  ...
  Future<void> moveQuestionsTo({
    ...
    await _db.transaction(() async {
      if (categoryId != null) {
        ...
        await (_db.delete(
          _db.questionCategories,
        )..where((qc) => qc.questionId.isIn(questionIds))).go();
        await _db.batch((b) {
          b.insertAll(_db.questionCategories, [
      ...
      if (subcategoryId != null) {
        await (_db.delete(
          _db.questionSubcategories,
        )..where((qs) => qs.questionId.isIn(questionIds))).go();
        await _db.batch((b) {
          b.insertAll(_db.questionSubcategories, [
            for (final q in questionIds)
              QuestionSubcategoriesCompanion.insert(
  ...
  Future<void> randomAssign({
    List<int> categoryIds = const [],
    List<int> subcategoryIds = const [],
    int perCategory = 0,
    int perSubcategory = 0,
  }) async {
    await _db.transaction(() async {
