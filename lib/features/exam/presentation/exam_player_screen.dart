...
class _ExamPlayerScreenState extends ConsumerState<ExamPlayerScreen> {
  ...
  void initState() {
    ...
    ref
        ...
        .event('exam_start', params: {'examId': widget.examId});
    load();
  }
  ...
  Future<void> load() async {
    try {
      ...
      if (widget.attemptId != null) {
        ...
        attemptId = await repo.startAttempt(
          examId: examId,
          mode: mode,
          userEmail: email,
        );
      ...
      if (list.isEmpty) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('No questions available for this exam yet'),
            ),
          );
      ...
      if (attemptId != null && widget.attemptId != null) {
        ...
        selections.addAll({
          for (final e in saved.entries) e.key: e.value.toSet(),
        });
      }
      setState(() {
      ...
      if (widget.categoryId != null) {
        final user = ref.read(currentUserProvider);
        final email = user?.email ?? 'guest@local';
        final saved = await repo.practiceProgress(
          ...
          WidgetsBinding.instance.addPostFrameCallback((_) async {
            ...
            final cont = await showDialog<bool>(
              ...
              builder: (ctx) => AlertDialog(
                ...
                content: Text(
                  'You stopped at question ${saved + 1} of ${questions0.length}. Continue or start over?',
                ),
                actions: [
                  TextButton(
            ...
            if (cont == true) {
              ...
              await repo.resetPracticeProgress(
                categoryId: widget.categoryId!,
                userEmail: email,
              );
              await _syncPendingReminder();
          ...
          IconButton(
            ...
            onPressed: () async {
              ...
              await ref
                  ...
                  .toggleSaved(questionId: q.id, userEmail: email);
              queueSync(email);
              if (context.mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('Updated saved questions')),
                );
              }
            },
          ),
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                        ...
                        if (mode == 'practice' && revealed.contains(q.id))
                          Padding(
                            ...
                            child: AnimatedContainer(
                              ...
                              decoration: BoxDecoration(
                                color: _isSelectionCorrect(selected, correctIds)
                                    ? Colors.green.withValues(alpha: 0.1)
                                    : Colors.red.withValues(alpha: 0.1),
                                borderRadius: BorderRadius.circular(12),
                              ),
                              child: Text(
                                q.explanation.isEmpty ? ' ' : q.explanation,
                              ),
                            ),
                          ),
                      ],
                ...
                Row(
                  children: [
                    OutlinedButton(onPressed: null, child: const Text('Skip')),
                    const Spacer(),
                    Expanded(
                      ...
                      child: FilledButton(
                        ...
                        onPressed: () async {
                          ...
                          if (attemptId != null) {
                            ...
                            final ok = _isSelectionCorrect(sel, correct);
                            if (sel.isNotEmpty) {
                              await ref
                                  .read(examRepositoryProvider)
                                  .saveAnswer(
                        ...
                        child: Text(() {
                          if (mode == 'practice') {
                            return !revealed.contains(q.id)
                                ? 'Check'
                                : (index < questions0.length - 1
                                      ? 'Next'
                                      : (skipped.isNotEmpty
                                            ? 'Review Skipped'
                                            : 'Submit'));
                          }
                          return index < questions0.length - 1
                              ? 'Next'
                              : (skipped.isNotEmpty
                                    ? 'Review Skipped'
                                    : 'Submit');
  ...
  bool _isSelectionCorrect(Set<int> selected, Set<int> correct) {
