            ...
            return FutureBuilder(
              future: () async {
                final cat = await (db.select(
                  db.categories,
                )..where((c) => c.id.equals(ex.categoryId))).getSingleOrNull();
                Subcategory? sub;
                if (ex.subcategoryId != null) {
                  sub =
                      await (db.select(db.subcategories)
                            ..where((s) => s.id.equals(ex.subcategoryId!)))
                          .getSingleOrNull();
                }
                final user = ref.read(currentUserProvider);
                bool isPro = false;
                ...
                return Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      Container(
                        ...
                        child: Row(
                          children: [
                            CircleAvatar(
                              radius: 26,
                              child: Icon(_iconForExam(ex.id)),
                            ),
                            const SizedBox(width: 12),
                            Expanded(
                              child: Text(
                                sanitizeDisplayText(ex.title),
                                style: Theme.of(
                                  context,
                      ...
                      Text(
                        '#questions: ${ex.questionCount}   -   Time: ${ex.timeLimitMinutes} mins   -   Pass: ${ex.passPercent}%',
                      ),
                      if (ex.description.isNotEmpty) ...[
                        const SizedBox(height: 8),
                        Text(sanitizeDisplayText(ex.description)),
                      ],
                      if ((ex.pdfUrl).isNotEmpty) ...[
                        const SizedBox(height: 12),
                        FutureBuilder<
                          ({bool readOnly, int page, String email})
                        >(
                          future: () async {
                            final user = ref.read(currentUserProvider);
                            final email = user?.email ?? 'guest@local';
                            ...
                            final page = await repo.pdfProgress(
                              examId: id,
                              userEmail: email,
                            );
                                            ...
                                            child: FilledButton(
                                              onPressed: () async {
                                                ...
                                                if (existing != null) {
                                                  ...
                                                  final choice =
                                                      await showDialog<String>(
                                                        ...
                                                        builder: (ctx) => AlertDialog(
                                                          ...
                                                          content: const Text(
                                                            'You have an ongoing assignment. Resume or start over?',
                                                          ),
                                                          actions: [
                                                            TextButton(
                                                              onPressed: () =>
                                                  ...
                                                  if (choice == 'resume') {
                                                    context.go(
                                                      '/player/$examId?aid=${existing.id}&mode=assignment',
                                                    );
                                                  } else if (choice == 'new') {
                                                    context.go(
                                                      '/player/$examId?mode=assignment',
                                                    );
                                                  }
                                                } else {
                                                  if (!context.mounted) return;
