...
class SyncRepository {
    ...
    await _db.transaction(() async {
        ...
        await _db
            ...
            .insert(
              ExamsCompanion.insert(
                ...
                questionCount: Value(
                  (m['question_count'] as num?)?.toInt() ?? 0,
                ),
                ...
                negativeMarking: Value(
                  (m['negative_marking'] as bool?) ?? false,
                ),
                passPercent: Value((m['pass_percent'] as num?)?.toInt() ?? 60),
      ...
      if (users.isNotEmpty) {
        ...
        for (final m in users) {
          ...
          await (_db.update(_db.users)..where((u) => u.email.equals(email)))
              .write(UsersCompanion(isPro: Value(isPro), role: Value(role)));
        }
      }
    });
    // Store content version if provided
    final version = (snap['version'] ?? DateTime.now().toIso8601String())
        .toString();
    await _db
        .into(_db.appSettings)
        .insertOnConflictUpdate(
          AppSettingsCompanion(
            key: Value('content_version'),
            value: Value(version),
          ),
        );
  ...
  Future<String?> localVersion() async {
    final row = await (_db.select(
      _db.appSettings,
    )..where((s) => s.key.equals('content_version'))).getSingleOrNull();
    return row?.value;
  }
    ...
    return {
      ...
      'users': [
        for (final u in await _db.select(_db.users).get())
          {'email': u.email, 'role': u.role, 'is_pro': u.isPro},
      ],
...
final syncRepositoryProvider = Provider<SyncRepository>(
  (ref) => SyncRepository(ref.watch(dbProvider), ref.watch(syncApiProvider)),
);
